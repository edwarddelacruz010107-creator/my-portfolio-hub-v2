...
**Phase 10 production release candidate completed on 2026-07-15; production promotion remains gated.** Trusted proxy handling is explicit and fail-safe; correlation/query telemetry is privacy-safe; sensitive proofs cross a fail-closed quarantine scanner; no CSP directive permits `unsafe-inline`; exact rendered attribute hashes preserve audited legacy behavior; all used Iconify assets are self-hosted; the 46-occurrence DOM sink inventory is classified and digest-locked; empty/drifted databases fail startup instead of crashing a request; local setup applies both migration histories with one command; legacy `init-db` schema creation is rejected; and the canonical Render target is a non-root Docker image with ClamAV and readiness probes. Render rehearsals exposed a misplaced `0057` ledger-backfill constraint plus ClamAV exhaustion/logging failures on the 512 MiB tier; the constraint is attached to the correct table, build-time signature loading is removed, non-root scanner paths are fixed, and the canonical service now uses the Standard 2 GiB tier. The source/domain gates pass 140 unittests, sixteen Phase 6 functions, seven DOM tests, nine token files, five themes, compilation, a 123-component SBOM, and the deterministic release gate. Exact-image PostgreSQL migration retry and Standard-tier scanner startup, broader PostgreSQL/Redis integration, browser/accessibility/performance matrices, EICAR/timeout tests, backup/PITR, rolling rollback, dependency/SAST/container scans, and an authorized staging penetration test remain mandatory promotion gates. See `PRODUCTION_RELEASE_CANDIDATE.md`, `PHASE_10_RELEASE_HARDENING.md`, and `SECURITY_OPERATIONS_RUNBOOK.md`.
