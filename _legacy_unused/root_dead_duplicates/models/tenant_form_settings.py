...
Security:
    ...
    - Encryption reuses the same Fernet helper from portfolio.py
"""
...
class TenantFormSettings(db.Model):
    ...
    Provider routing:
        'basin'     → POST to form_endpoint with api_key as URL param
    ...
    form_endpoint     = db.Column(db.Text,        nullable=True)   # Basin: full URL; Web3Forms: None (static)
    ...
    def api_key(self, value: str) -> None:
        """Encrypt and store API key. Pass '' to clear."""
        self._api_key_encrypted = encrypt_secret(value) if value else ''
