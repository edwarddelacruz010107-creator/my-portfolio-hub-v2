...
def settings():
    ...
    from app.services.billing.countries import country_details, country_options
    from app.services.billing.currency import SUPPORTED_CURRENCIES, currency_context

    selected_country = country_details(getattr(tenant, 'country_code', None) or 'PH') if tenant else country_details('PH')
    selected_currency = (getattr(tenant, 'preferred_currency', None) or selected_country['currency']).upper() if tenant else selected_country['currency']
    if selected_currency not in SUPPORTED_CURRENCIES:
        selected_currency = selected_country['currency']
    try:
        tenant_currency = currency_context(target=selected_currency)
    except Exception:
        logger.exception('Unable to load tenant currency preview')
        tenant_currency = {
            'base_currency': 'USD', 'display_currency': selected_currency,
            'symbol': SUPPORTED_CURRENCIES.get(selected_currency, {}).get('symbol', selected_currency + ' '),
            'fx': {'rate': 1.0, 'available': selected_currency == 'USD', 'stale': True},
        }

    return render_template(
        'admin/settings.html', form=form, username_form=username_form,
        tenant=tenant, form_settings=form_settings,
        country_options=country_options(), selected_country=selected_country,
        supported_currencies=SUPPORTED_CURRENCIES,
        selected_currency=selected_currency, tenant_currency=tenant_currency,
        auto_currency=(getattr(tenant, 'country_source', '') != 'tenant_manual_currency') if tenant else True,
    )

@admin.route('/settings/region-currency', methods=['POST'])
@admin_required
@limiter.limit('10 per minute')
def update_region_currency():
    """Save the tenant's billing country and display currency.

    Plan prices remain stored in USD. This setting only controls the tenant's
    regional billing presentation and the currency suggested at checkout.
    """
    from app.services.billing.countries import (
        COUNTRIES, country_details, currency_for_country, normalize_country_code,
    )
    from app.services.billing.currency import SUPPORTED_CURRENCIES

    tenant = tenant_repository.get_by_slug(_active_tenant_slug())
    if tenant is None:
        flash('Tenant account could not be found.', 'danger')
        return redirect(url_for('admin.settings'))

    submitted_country = (request.form.get('country_code') or '').strip().upper()
    # Reject invalid values instead of silently changing the user's selection.
    if submitted_country not in COUNTRIES:
        flash('Please select a valid country.', 'danger')
        return redirect(url_for('admin.settings') + '#region-currency')

    country_code = normalize_country_code(submitted_country)
    auto_currency = request.form.get('auto_currency') == '1'
    requested_currency = (request.form.get('preferred_currency') or '').strip().upper()

    if auto_currency:
        preferred_currency = currency_for_country(country_code)
        source = 'tenant_auto'
    else:
        if requested_currency not in SUPPORTED_CURRENCIES:
            flash('Please select a supported billing currency.', 'danger')
            return redirect(url_for('admin.settings') + '#region-currency')
        preferred_currency = requested_currency
        source = 'tenant_manual_currency'

    old_country = tenant.country_code
    old_currency = tenant.preferred_currency
    tenant.country_code = country_code
    tenant.preferred_currency = preferred_currency
    tenant.country_source = source
    ...
    try:
        ...
        db.session.rollback()
        logger.exception('Failed to update tenant region and currency')
        flash('Region and currency settings could not be saved. Please try again.', 'danger')
        return redirect(url_for('admin.settings') + '#region-currency')

    details = country_details(country_code)
    log_activity(
        'update', 'tenant', tenant.slug,
        f"Billing region changed from {old_country or 'unset'}/{old_currency or 'unset'} to {country_code}/{preferred_currency}",
        tenant_slug=tenant.slug,
    )
    flash(f"Region updated to {details['name']}. Billing prices will display in {preferred_currency}.", 'success')
    return redirect(url_for('admin.settings') + '#region-currency')
