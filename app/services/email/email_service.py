...
def _build_otp_email_body(otp: str, user_type: str, ip_address, user_agent, ttl_minutes: int):
    """Build (subject, text, html) for an OTP password-reset email. Shared by
    send_otp_email() (global MailerSend/SMTP path) and any tenant-owned
    delivery path so the email content stays identical regardless of which
    provider chain actually sends it."""
...
def send_otp_email(
    ...
    subject, text, html = _build_otp_email_body(otp, user_type, ip_address, user_agent, ttl_minutes)
