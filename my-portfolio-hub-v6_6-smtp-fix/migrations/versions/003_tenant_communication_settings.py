...
down_revision = '0018_auth_otp_web3forms'   # Anchored: tenant_communication_settings depends on tenants + GlobalEmailConfig
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # FIXED: 'tenant_communication_settings' is now created by
    # 0001_initial_schema.py.
    if not inspector.has_table('tenant_communication_settings'):
        op.create_table(
    ...
    existing_constraints = {c['name'] for c in inspector.get_unique_constraints('tenant_communication_settings')}
    if 'uq_tenant_comm_settings' not in existing_constraints:
    ...
    existing_indexes = {ix['name'] for ix in inspector.get_indexes('tenant_communication_settings')}
    if 'ix_tenant_comm_slug' not in existing_indexes:
