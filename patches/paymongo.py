"""
app/utils/paymongo.py — PayMongo automated billing integration (v3.9 patched)

Patches applied:
  • BUG-003: hmac.new() → hmac.HMAC() (forward-compatible, explicit)
  • BUG-008: verify_webhook_signature() now correctly parses PayMongo's
            compound signature header format: t=<ts>,te=<hmac_te>,li=<hmac_li>
  • BUG-014: create_payment_source() now passes db_session and tenant_id correctly
  • BUG-015: Webhook returns 200 on internal errors to prevent PayMongo retry storms
             (moved to webhooks/__init__.py patch)
"""
...
def get_payment_intent(payment_intent_id: str) -> dict | None:
    """
    Fetch a PayMongo PaymentIntent by ID.
    Added for BUG-005 — was referenced but never defined.
    """
    result = _api_request('GET', f'/payment_intents/{payment_intent_id}')
...
def verify_webhook_signature(payload: bytes, signature_header: str) -> bool:
    ...
    BUG-008 FIX: PayMongo's Paymongo-Signature header has the format:
        t=<unix_timestamp>,te=<hmac_of_timestamp.payload>,li=<hmac_of_payload>

    The `li` field is HMAC-SHA256(secret, raw_payload) as a hex digest.
    The previous code treated the entire header value as a bare hex digest,
    which would always fail in production.
    ...
    # If the header is a bare hex digest (legacy / test mode), fall through to direct compare
    if not parts:
        # Legacy: treat the whole header as the hex digest (for local testing)
        try:
            # BUG-003 FIX: use hmac.HMAC() instead of deprecated hmac.new()
            expected = hmac.HMAC(
    ...
    # Standard PayMongo: verify using the `li` field
    li_signature = parts.get('li', '')
    if not li_signature:
        logger.warning('PayMongo webhook: signature header missing `li` field')
        return False
    ...
    try:
        ...
        return hmac.compare_digest(expected, li_signature.lower())
    except Exception:
        logger.exception('PayMongo HMAC verification error')
        return False
...
def _handle_subscription_updated(attrs: dict, _event_id: str, event_type: str) -> bool:
    ...
    # BUG-004 FIX: import both functions that now exist in billing.py
...
def _handle_subscription_cancelled(attrs: dict, _event_id: str, event_type: str) -> bool:
    from app.models.portfolio import Subscription
    # BUG-004 FIX: now exists in billing.py
...
def create_payment_source(
    ...
    """
    BUG-014 FIX: now correctly passes db.session and tenant_id to
    get_or_create_pending_subscription() instead of (profile, plan).
    """
    ...
    sub = get_or_create_pending_subscription(
        db.session,                    # BUG-014 FIX: pass db_session
        tenant_id,                     # BUG-014 FIX: pass tenant_id, not profile
    ...
    if result:
        sub.paymongo_id = result.get('session_id')  # BUG-002 FIX: use paymongo_id
        db.session.commit()
    return result
